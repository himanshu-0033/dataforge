...
The API entry point is `counselor.app:app` under `pickmate/backend`; the worker is `counselor.worker`. It uses a separate `heard` agent and room prefix. The React entry point is `pickmate/web/src/counselor/CounselorApp.tsx`. PickMate remains a separately runnable frontend at `/pickmate.html`, with its own inventory API and worker; see `pickmate/README.md`. The default API port is shared, so use the launch commands for the app being demonstrated. The earlier root browser experiment remains as a source reference.
