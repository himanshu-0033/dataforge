...
export default function App() {
  ...
  const presence = snapshot?.user_speaking ? 'listening' : speaking ? 'speaking' : snapshot?.thinking ? 'thinking' : 'still';
    ...
    <AnimatePresence mode="wait">
      {!credential ? <motion.main id="main" key="welcome" className="heard-welcome" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .15 }}>
        ...
        <section className="heard-conversation" aria-label="Conversation with Heard">
          ...
          {snapshot && <SessionPreferences support={snapshot.support || 'explore'} focus={snapshot.focus || ''} busy={busy} saving={pending === 'preferences'} onChange={preferences}/>}
          ...
          {quiet && <section className={`heard-quiet ${presence}`} aria-label="Quiet conversation space">
            <div className="heard-presence" aria-hidden="true"><span/><span/><span/><Wave moving={presence === 'speaking' || presence === 'listening'}/></div>
            <span className="heard-quiet-label">Heard <small>AI support</small></span>
            <p aria-live={snapshot?.draft ? 'off' : 'polite'}>{snapshot?.draft || (snapshot?.thinking ? 'Taking a moment with what you shared…' : latestReply?.text || 'There’s room to take your time.')}</p>
            {snapshot?.thinking && <ThinkingDots/>}
          </section>}
          <div className="heard-transcript" hidden={quiet} ref={transcript} role="log" aria-label="Conversation messages" aria-live="polite" aria-relevant="additions text" onScroll={event => { const el = event.currentTarget; follow.current = el.scrollHeight - el.scrollTop - el.clientHeight < 100; }}>
