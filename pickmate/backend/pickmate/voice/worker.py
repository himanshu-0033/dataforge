"""Long-running LiveKit worker. The API alone interprets turns and owns task state."""
...
def eligible_speech(snapshot):
    ...
    if snapshot.get("ended") or snapshot.get("resolving") or not speech:
        return None
    if snapshot.get("paused") and not speech.get("allow_while_paused", False):
        return None
    ...
    if speech.get("task_id") and (
        not task or speech["task_id"] != task["task_id"] or speech.get("task_version") != task["task_version"]
    ):
        return None
    return speech


def inventory_keyterms(snapshot):
    terms = []
    for item in snapshot.get("inventory", []):
        for term in [item.get("name", ""), *item.get("aliases", [])]:
...
class Bridge:
    ...
    def delivery_estimate(self, item):
        if item.role != "assistant":
            return
        for handle, identity in self.handles.values():
            if any(message.id == item.id for message in handle.chat_items):
                self.submit(
                    "metrics",
                    {
                        ...
                        "data": {
                            **identity,
                            "sdk_speech_id": handle.id,
                            "sdk_message_id": item.id,
                            "text": item.text_content,
                            "interrupted": item.interrupted,
                            "estimate": "SDK aligned/forwarded transcript; physical hearing unverified",
                            "trusted": False,
                            "worker_monotonic": time.monotonic(),
                        },
                    },
    ...
    async def playback(self, response_id, status):
        await self.call("playback", {"response_id": response_id, "status": status, "trusted": False})
    ...
    async def silence_fallback(self, serial):
        ...
        # silent lookup also needs recovery after VAD-only noise.
        ...
        while not self.closed:
            try:
                ...
                if speech and not self.failed and speech["response_id"] not in self.seen:
                    ...
                    self.handles[self.handle.id] = (
                        self.handle,
                        {
                            key: speech.get(key)
                            for key in ("response_id", "response_epoch", "task_id", "task_version")
                        },
                    )
...
async def entrypoint(ctx: agents.JobContext):
    ...
    async with httpx.AsyncClient(timeout=5) as http:
        ...
        stt = deepgram.STT(
            ...
            keyterms=inventory_keyterms(snapshot),
        )
        ...
        def user_state(event):
            ...
            bridge.submit(
                ...
                {
                    "type": "user_state",
                    "data": {"state": event.new_state, "worker_monotonic": time.monotonic()},
                },
            )
        ...
        def agent_state(event):
            ...
            identity = bridge.handles.get(handle.id) if handle else None
            if event.new_state == "speaking" and identity:
                bridge.submit(
                    "playback",
                    {"response_id": identity[1]["response_id"], "status": "playing", "trusted": False},
                )
                bridge.spawn(bridge.provider("active"))

        @session.on("metrics_collected")
        def metrics(event):
            data = event.metrics.model_dump(mode="json")
            bridge.submit("metrics", {"type": "livekit_metrics", "data": data})

        @session.on("conversation_item_added")
        def conversation_item(event):
            bridge.delivery_estimate(event.item)
        ...
        try:
            ...
            ctx.shutdown(reason="pickmate_worker_failed" if bridge.failed else "pickmate_worker_closed")
